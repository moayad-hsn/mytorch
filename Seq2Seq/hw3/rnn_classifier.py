...
class RNNPhonemeClassifier(object):
    ...
    def forward(self, x, h_0=None):
        ...
        for seq in range(seq_len):
            for ix, layer in enumerate(self.rnn):
                if ix == 0:
                    hidden[ix] = layer.forward(self.x[:, seq, :], hidden[ix])
                else:
                    hidden[ix] = layer.forward(hidden[ix-1], hidden[ix])
            self.hiddens.append(hidden.copy())
        ...
        for seq in range(seq_len-1, -1, -1):
            for layer in range(self.num_layers-1, -1, -1):
                if layer == 0:
                    ...
                    h_prev_l = self.hiddens[seq+1][layer-1]
                
                h_prev_t = self.hiddens[seq][layer]

                h_current_t = self.hiddens[seq+1][layer]

                dx_out, dh_out = self.rnn[layer].backward(dh[layer], h_current_t, h_prev_l, h_prev_t)
